export const LEFT_NAVBAR_ITEMS = [
  {
    link: "/about",
    name: "About",
  },
  {
    link: "/contact",
    name: "Contact",
  },
];

export const RIGHT_NAVBAR_ITEMS = [
  {
    link: "/login",
    name: "Login",
  },
  {
    link: "/signup",
    name: "Sign Up",
  },
];

export const HERO_ITEMS = {
  heading: "Creovo",
  sub_heading:
    "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quod.",
};

export const TIMELINE_ITEMS = {
  heading: "Our working timeline",
  sub_heading:
    "We&apos;ve been working on Website development field for the past 4 years. Here&apos;sa timeline of our workflow.",
};

export const TECHSTACK_ITEMS = {
  heading:
    "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quod.",
  sub_heading:
    "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quod.",
};

export const MILESTONES_DATA = {
  heading: "Our Milestones",
  sub_heading: "A journey of innovation and success.",
  launches: [
    {
      title: "Project Alpha",
      description:
        "The first major step in our journey, laying the foundation for future innovations.",
      mission_number: "01",
    },
    {
      title: "Beta Launch",
      description:
        "Introducing our platform to a select group of users and gathering valuable feedback.",
      mission_number: "02",
    },
    {
      title: "Public Release",
      description:
        "The official launch of our platform, making it available to the world.",
      mission_number: "03",
    },
    {
      title: "Global Expansion",
      description:
        "Expanding our reach to new markets and connecting with a global audience.",
      mission_number: "04",
    },
  ],
};

export const CTA_DATA = {
  heading: "Ready to Get Started?",
  sub_heading:
    "Join us today and take the first step towards a brighter future.",
  CTAs: [
    {
      URL: "/contact",
      text: "Contact Us",
      variant: "primary",
    },
    {
      URL: "/pricing",
      text: "View Pricing",
      variant: "muted",
    },
  ],
};

export const TESTIMONIALS_DATA = {
  heading: "What Our Clients Say",
  sub_heading: "Hear from those who have experienced our service first-hand.",
  testimonials: [
    {
      quote:
        "This service has transformed the way we do business. The results have been phenomenal.",
      user: {
        firstname: "John",
        lastname: "Doe",
        image: {
          url: "data:image/jpeg;base64,",
        },
        job: "CEO, Innovate Inc.",
      },
      title: "CEO, Innovate Inc.",
      description: 'This is one of the best agency. Integrating LaunchPad into our workflow was seamless. The advanced analytics and reporting tools give us valuable insights, helping us to continuously improve our content strategy.'
    },
    {
      quote:
        "This service has transformed the way we do business. The results have been phenomenal.bcdskjbckjbsdjkbcjksdbcsdb ckjbsdjkcbjsdkbcjbdskhcbdskjbcjksdbckjbdsckjbsdjkcbdj skbcjkdsbjckbsddkjcbdsjksbckjds bcjkbdjkcbdskjcbkds bcjdsbkcjsd",
      user: {
        firstname: "John",
        lastname: "Doe",
        image: {
          url: "data:image/jpeg;base64,",
        },
        job: "CEO, Innovate Inc.",
      },
      title: "CEO, Innovate Inc.",
      description: 'This is one of the best agency. Integrating LaunchPad into our workflow was seamless. The advanced analytics and reporting tools give us valuable insights, helping us to continuously improve our content strategy.'
    },
    {
      quote:
        "This service has transformed the way we do business. The results have been phenomenal.",
      user: {
        firstname: "John",
        lastname: "Doe",
        image: {
          url: "data:image/jpeg;base64,",
        },
        job: "CEO, Innovate Inc.",
      },
      title: "CEO, Innovate Inc.",
      description: 'This is one of the best agency. Integrating LaunchPad into our workflow was seamless. The advanced analytics and reporting tools give us valuable insights, helping us to continuously improve our content strategy.'
    },
  ],
};
